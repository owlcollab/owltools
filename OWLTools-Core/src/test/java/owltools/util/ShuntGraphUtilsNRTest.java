...
public class ShuntGraphUtilsNRTest extends OWLToolsTestBasics {
	...
	private void checkTopologyRelationsUpper(final OWLGraphWrapper g, OWLShuntGraph shunt, Set<OWLShuntEdge> remaining) {
		...
		containsNotRel("GO:0044451", "GO:0005575", "rdfs:subClassOf", shunt, g, remaining); // REDUNDANT nucleoplasm part -> cellular_component
		...
		containsNotRel("GO:0044424", "GO:0005575", "rdfs:subClassOf", shunt, g, remaining); // REDUNDANT intracellular part -> cellular_component
	}
	...
	private void containsNotRel(String s, String t, String r, OWLShuntGraph shuntGraph, OWLGraphWrapper graph, Set<OWLShuntEdge> remaining) {
	    boolean found = false;
	    OWLObject cs = graph.getOWLObjectByIdentifier(s);
	    OWLObject ct = graph.getOWLObjectByIdentifier(t);
	    for (OWLShuntEdge edge : shuntGraph.edges) {
	    ...
	    assertFalse("Found unexpected edge: ("+
	            graph.getLabelOrDisplayId(cs)+"; "+
	            graph.getLabelOrDisplayId(ct)+"; "+
	            r+")", found);
	}
