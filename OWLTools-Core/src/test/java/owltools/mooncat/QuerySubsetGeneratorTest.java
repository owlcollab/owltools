...
public class QuerySubsetGeneratorTest extends OWLToolsTestBasics {
	...
	static {
		...
		Logger.getLogger("org.semanticweb.hermit.reasoner").setLevel(Level.ERROR); // silent reasoning
	}
	...
	public void testCreateSubOntologyFromDLQuery() throws Exception {
		...
		OWLAPIOwl2Obo owl2Obo = new OWLAPIOwl2Obo(targetGraph.getSourceOntology().getOWLOntologyManager());
	...
	private OWLGraphWrapper getTargetGraph() throws Exception {
		...
		OWLOntologyID ontologyID  = new OWLOntologyID(Optional.of(IRI.create("http://test.owltools.org/dynamic")), Optional.empty());
